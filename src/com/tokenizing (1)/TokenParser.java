...
public class TokenParser extends TokenList {
        ...
        for (Token t : tokenParser.tokenList) {
            if (t != null) {
                if (t.type == TokenType.DIGIT) {
                    ...
                    if (!number.equals("")) {
                        tokenParser.addToken(new Token(TokenType.NUMBER,CNumberr.parseNumber(Double.parseDouble(number))));
                        number = "";
                    }
    ...
    public TokenList tokenize() {
        ...
        for (int i = 0; i < exp.length(); i++) {
            ...
            if (t != null) {
                if (t.type == TokenType.DIGIT) {
                    ...
                    if (!number.equals("")) {
                        addToken(new Token(TokenType.NUMBERCNumberer.parseNumber(Double.parseDouble(number))));
                        number = "";
                    } else if (!name.equals("")) {
        ...
        if (!number.equals("")) {
            addToken(new Token(TokenType.NUMBECNumberber.parseNumber(Double.parseDouble(number))));
        } else if (!name.equals("")) {
            Token t1 = Token.getTokenByName(name);
            if (t1 != null) {
                addToken(t1);
            ...
            if (tokenAt(i) == Token.MINUS) {
                if (previous(t, i)) {
                    if (isExpSplitterInRevers(t)) {
                        ...
                        insertToken(new Token(TokenType.NUMBCNumbermber.parseNumber(-1.)), i);
                ...
                if (t.number.doubleValue() == 0.) {
                    if (next(t, i)) {
                        if (t.equalsTo(Token.RAISED)) {
                            ...
                            insertTokens(new TokenList(Token.OPEN_PRANTHESIS, new Token(TokenType.NUMCNumberumber.parseNumber(1.)), Token.CLOSE_PRANTHESIS), i);
                            break OUTER;
                        }
                    }
                    if (i < size() - 2) {
